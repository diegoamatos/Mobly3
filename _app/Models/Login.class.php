...
class Login {
    ...
    public function CheckLogin() {
        if (empty($_SESSION['userlogin']) || $_SESSION['userlogin']['nivel_usuario'] < $this->Level):
    ...
    private function setLogin() {
        ...
        elseif (!$this->getUser()):
            ...
            $this->Result = false;
        elseif ($this->Result['nivel_usuario'] < $this->Level):
            $this->Error = ["Desculpe {$this->Result['nome_usuario']}, você não tem permissão para acessar esta área!", WS_ERROR];
            $this->Result = false;
        else:
            $this->Execute();
        endif;
    ...
    private function getUser() {
        ...
        $read = new Read;
        $read->ExeRead("mb_usuario", "WHERE email_usuario = :e AND senha_usuario = :p", "e={$this->Email}&p={$this->Senha}");
    ...
    private function Execute() {
        ...
        $this->Error = ["Olá {$this->Result['nome_usuario']}, seja bem vindo(a). Aguarde redirecionamento!", WS_ACCEPT];
        $this->Result = true;
    }
